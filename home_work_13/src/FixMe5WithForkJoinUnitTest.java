            ...
            public void run() {
                for (int i = 0; i < 1000; i++) {
                    try {
                        ...
                        class SumRecursiveTask extends RecursiveTask<BigDecimal> {
                            ...
                            protected BigDecimal compute() {
                                if (list.size() <= 16) {
                                    ...
                                    List<Integer> leftList = list.subList(0, mediana);
                                    List<Integer> rightList = list.subList(mediana, list.size() - 1);

                                    SumRecursiveTask leftTask = new SumRecursiveTask(leftList);
                                    leftTask.fork();
                                }
                            }
                        }
                        ...
                        BigDecimal bigDecimal = pool.invoke(new SumRecursiveTask(list));
