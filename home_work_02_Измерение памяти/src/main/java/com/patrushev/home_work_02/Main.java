...
import com.patrushev.home_work_02.factories.ObjectFactoryByCloning;
...
public class Main {
    public static void main(String[] args) {
        ...
        emptyStringObjectMemorySize = ObjectSizeDeterminer.getMemorySizeOfObject(new ObjectFactoryByCloning(""));
        System.out.println("Universal fabric - Memory size of empty String is " + emptyStringObjectMemorySize + " bytes");
        intArrayMemorySize = ObjectSizeDeterminer.getMemorySizeOfObject(new ObjectFactoryByCloning(new int[0]));
        System.out.println("Universal fabric - Memory size of int array (length is 0) is " + intArrayMemorySize + " bytes");
        intArrayMemorySize = ObjectSizeDeterminer.getMemorySizeOfObject(new ObjectFactoryByCloning(new int[4]));
        System.out.println("Universal fabric - Memory size of int array (length is 4) is " + intArrayMemorySize + " bytes");
        intArrayMemorySize = ObjectSizeDeterminer.getMemorySizeOfObject(new ObjectFactoryByCloning(new int[8]));
        System.out.println("Universal fabric - Memory size of int array (length is 8) is " + intArrayMemorySize + " bytes\n");

        //инструментирование
        emptyStringObjectMemorySize = InstrumentationAgent.getObjectSize("");
